...
def historico_restaurante(nif):
    ...
    hist = [p for p in pedidos if p["Restaurante_NIF"] == nif]
...
def list_restaurantes():
    ...
    df = df[["Nome", "NIF", "Preco", "Avaliacão", "Tipo", "Rating", "Localizacao", "Lat","Lon"]]
    display(df)
...
def listar_pedidos_cliente(id_cliente):
    ...
    pedidos_cliente = [p for p in pedidos if p["Cliente_ID"] == id_cliente]
...
def tabela_avaliacao():
    ...
    df = pd.DataFrame([{**{"NIF": nif}, **dados} for nif, dados in restaurantes.items()])
    df = df[["Nome", "NIF", "Avaliacão"]] # Corrected here
    display(df)
